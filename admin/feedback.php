...
{
$userid=$_GET['uid'];
$query=mysqli_query($con,"delete from feedback2 where ID='$userid'");
                ...
                <tbody>
                   <tr>
                       ...
                       $page = $_GET['page'];
                        ...
                        $con= new mysqli("localhost","root","","sonet");
                           $page = 1;
                       $doc_query = "select * from feedback2";
                       $total = mysqli_query($con,$doc_query);
                       $items_per_page = 10;
                       $total_links = ceil(mysqli_num_rows($total)/$items_per_page);
                       $offset = ($page-1) * $items_per_page;
                       $sql_command = "select * from feedback2 limit $items_per_page OFFSET $offset";
                       ...
                       while($row = mysqli_fetch_assoc($list)){
                         ...
                         <!--   <a class="btn btn-primary btn-sm" role="submit" href="edit.php?id=<?php echo $row['id']?>" ><i class="icon-edit">Edit</i></a> -->
                           <a class="btn btn-danger btn-sm " role="submit" href="feedback.php?uid=<?php echo htmlentities($row['ID']);?>&&action=del" title="Delete" onClick="return confirm('Do you really want to delete the Message ?')">
